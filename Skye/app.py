...
@app.route('/api/vrt-calculate', methods=['POST'])
def vrt_calculate():
    ...
    try:
        ...
        try:
            rate_response = requests.get('https://api.exchangerate-api.com/v4/latest/GBP', timeout=10)
        ...
        co2_bands = [
            ...
            (101, 105, 12.75, 255), (106, 110, 13.5, 270), (111, 115, 15.25, 305),
            (116, 120, 16, 320), (121, 125, 16.75, 335), (126, 130, 17.5, 350),
            (131, 135, 19.25, 385), (136, 140, 20, 400), (141, 145, 21.5, 430),
            (146, 150, 25, 500), (151, 155, 27.5, 550), (156, 170, 30, 600),
        ...
        total_import_cost = vehicle_value_eur + total_transport + customs_duty + final_vrt + vat_amount + registration_fee
...
@app.route('/api/music-next/search', methods=['GET'])
def music_next_search():
    ...
    try:
        ...
        app.logger.info(f"Music Next search requested for artist: {artist}")

        if not artist:
            app.logger.warning("Music Next search failed: No artist name provided")
        ...
        response = requests.get(url, headers=headers, timeout=10)
        if response.status_code != 200:
            app.logger.error(f"Music-map.com returned status {response.status_code} for artist: {artist}")
        ...
        if not similar_artists:
            app.logger.warning(f"No similar artists found for: {artist}")
            return jsonify({'error': 'No similar artists found'}), 404

        app.logger.info(f"Found {len(similar_artists)} similar artists for: {artist}")

        # Load or create music recommendations state
        config_path = os.path.join(os.path.dirname(__file__), 'music_recommendations.json')
        ...
        if artist_key in music_config['artists']:
            existing_artists = set(music_config['artists'][artist_key].get('all_recommendations', []))
            new_artists = [a for a in similar_artists if a not in existing_artists]

            # If re-searching, add new artists to the pending list
            if new_artists:
                music_config['artists'][artist_key]['all_recommendations'].extend(new_artists)
            ...
            music_config['artists'][artist_key] = {
                ...
                'listened': [],
                'history': [similar_artists[0]] if similar_artists else [],  # Track navigation history
        ...
        app.logger.error(f"Music Next search error for artist {artist}: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/music-next/listened', methods=['POST'])
def music_next_listened():
    """Mark a recommendation as listened"""
    try:
        app.logger.info("Music Next: Marking recommendation as listened")
        data = request.get_json()
        ...
        # Initialize history and current_index if not present (for backwards compatibility)
        ...
        # Check if we can navigate forward through existing history
...
@app.route('/api/music-next/skip', methods=['POST'])
def music_next_skip():
...
@app.route('/api/music-next/back', methods=['POST'])
def music_next_back():
    ...
    try:
        ...
        if artist_data['current_index'] <= 0:
            return jsonify({'error': 'No previous recommendations to go back to'}), 400
...
@app.route('/api/music-next/current', methods=['GET'])
def music_next_current():
    ...
    try:
        ...
        elif artist_data['history'] and artist_data['current_index'] < len(artist_data['history']):
...
def get_weather():
    ...
    try:
        ...
        app.logger.info(f"Weather data requested for coordinates: lat={lat}, long={long}")
        ...
        if response.status_code != 200:
            app.logger.error(f"Met Éireann API returned status {response.status_code}")
            return jsonify({'error': 'Failed to fetch weather data'}), 500

        app.logger.info("Successfully fetched weather data from Met Éireann")
...
@app.route('/api/music-next/history', methods=['GET'])
def music_next_history():
    ...
    try:
        ...
        history = [data['search_artist'] for data in music_config.get('artists', {}).values()]
...
@app.route('/api/music-next/artist-image', methods=['GET'])
def music_next_artist_image():
    ...
    try:
        ...
        wiki_search_url = f'https://en.wikipedia.org/w/api.php?action=query&format=json&prop=pageimages&piprop=original&titles={encoded_artist}'
        ...
        search_url = f'https://en.wikipedia.org/w/api.php?action=opensearch&format=json&search={encoded_artist}&limit=1'
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            if len(search_data) > 1 and len(search_data[1]) > 0:
                ...
                image_url = f'https://en.wikipedia.org/w/api.php?action=query&format=json&prop=pageimages&piprop=original&titles={encoded_title}'
        ...
        app.logger.error(f"Artist image fetch error: {str(e)}")
        return jsonify({'image_url': None})
...
def page_content(page_id):
    ...
    pages_dir = os.path.join(os.path.dirname(__file__), 'pages', page_id)
    ...
    if os.path.exists(page_file):
        try:
            spec = importlib.util.spec_from_file_location(f"{page_id}_page", page_file)
            page_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(page_module)
    ...
    return jsonify({'html': f'<h2>Page: {page_id}</h2><p>No content available</p>'})
